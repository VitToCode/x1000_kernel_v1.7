...
static struct pmu_platform_data hdmi_80_pmu_pdata = {
	.gpio = GPIO_PA(28),
	.num_regulators = ARRAY_SIZE(hdmi_80_pmu_regulators),
	.regulators = hdmi_80_pmu_regulators,
	.charger_board_info = &charger_board_info,
};

#define PMU_I2C_BUSNUM 0

struct i2c_board_info hdmi_80_pmu_board_info = {
	I2C_BOARD_INFO("act8600", 0x5a),
	.platform_data = &hdmi_80_pmu_pdata,
};
...
static int __init hdmi_80_pmu_dev_init(void)
{
	...
	client = i2c_new_device(adap, &hdmi_80_pmu_board_info);
	if (!client) {
