...
#ifdef CONFIG_MUL_PARTS
		j = 0;
		if((ptemp+ret)->ex_partition->size == 0){
			(g_partition+ret)->parts_num = 0;
		}
		while((ptemp+ret)->ex_partition->size != 0){
			if(((ptemp+ret)->ex_partition+j)->size != 0){
				((g_partition+ret)->parts+j)->size = div_s64_32(((ptemp+ret)->ex_partition+j)->size, 1);
				((g_partition+ret)->parts+j)->offset = div_s64_32(((ptemp+ret)->ex_partition+j)->offset, 1);
