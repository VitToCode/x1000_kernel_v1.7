...
static int alloc(struct MemoryDealer *dealer,int size, unsigned int flags){
	...
	bilist_for_each(pos,&dealer->top){
		cur = bilist_entry(pos,chunk_t,head);
        int extra = 0;
        ...
        if (free_size > size) {
            ...
            if (extra) {
				...
				if(split == NULL){
					...
					split->size = extra;
					split->free = 1;
					bilist_add_tail(&split->head,&free_chunk->head);
            ...
            if (tail_free > 0) {
				...
				if(split == NULL){
					...
					split->free = 1;
					bilist_add(&split->head,&free_chunk->head); //for s_alloc
				}
		    }
        }
...
static chunk_t* dealloc(struct MemoryDealer *dealer,size_t start)
{
	...
	bilist_for_each(pos,&dealer->top){
		...
		if (cur->start == start) {
			...
			flag = 1;
			cur->free = 1;				
			p = bilist_entry(pos->prev,chunk_t,head);
			n = bilist_entry(pos->next,chunk_t,head);
			
			if (!bilist_is_last(&p->head,&dealer->top) && (p->free || !cur->size)) {
				freed = p;
				p->size += cur->size;
				bilist_del(pos);
