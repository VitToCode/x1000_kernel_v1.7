...
static int simpblock_rw(SmbContext *conptr, SectorList *sl, int rwflag)
{
	...
	if ((rwflag == SIMP_WRITE) && lpl->startPageID == 0)
...
/**
 *	SimpleBlockManager_Erase  -  Erase partition
 *
 *	@context: return value of SimpleBlockManager_Open
 */
int SimpleBlockManager_Erase(int context)
{
	return 0;
}
...
PartitionInterface smb_nand_ops = {
	...
	.PartitionInterface_Erase	= SimpleBlockManager_Erase,
};
