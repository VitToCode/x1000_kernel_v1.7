...
int CacheManager_DropAllCache(int context)
{
	CacheManager *cachemanager = (CacheManager *)context;
        CacheData *cachedata = NULL;
        int i = 0;

        CacheData_Dropdata(cachemanager->L1Info);
        if (cachemanager->L2Info) {
                for(i = 0; i < L2_CACHEDATA_COUNT; i++) {
                        cachedata = CacheList_getTail(cachemanager->L2Info);
                        if(cachedata == NULL)
                                return -1;
                        CacheData_Dropdata(cachedata);
                        CacheList_Insert(cachemanager->L2Info,cachedata);
                }
        }
        if(cachemanager->L3Info) {
                for(i = 0; i < L3_CACHEDATA_COUNT; i++) {
        ...
        if(cachemanager->L4Info) {
                for(i = 0; i < L4_CACHEDATA_COUNT; i++) {
