...
*/
		...
		usecount = ZoneManager_Getusedcount(context);
                Recycle_Lock(context);
		recyclemode = rep->recyclemode;
                Recycle_Unlock(context);
                if (recyclemode) {
                        if (recyclemode & RECYCLE_FAST_MODE) {
				...
				if (Get_JunkZoneRecycleTrig(conptr->junkzone,percent)) {
					Recycle_OnNormalRecycle(context);
                        ...
                        if (recyclemode & RECYCLE_NORMAL_MODE) {
			...
			if (recyclemode & RECYCLE_ERASE_MODE) {
...
int L2PConvert_ZMOpen(VNandInfo *vnand, PPartition *pt)
{
	...
	conptr->junkzone = Init_JunkZone((int)conptr,zonecount);
...
static int Read_sectornode_to_pagelist(int context, int sectorperpage, SectorList *sectornode, PageList *pagelist)
{
	...
	for (i = sectorid; i < sectorid + sectorcount; i += (k + l)) {
		...
		pagenode = (PageList *)BuffListManager_getNextNode(conptr->blm, (void *)pagelist, sizeof(PageList));
...
static int start_ecc_error_handle(int context, unsigned int pageid)
{
	...
	*/
	...
	if(ret != -1)
                nm_set_bit(blockid - zone_start_blockid,
                                (unsigned int *)&(conptr->top + errinfo.err_zoneid)->badblock);
...
int L2PConvert_ReadSector ( int handle, SectorList *sl )
{
	...
	BuffListManager_freeList(conptr->blm, (void **)&pl, (void *)pl, sizeof(PageList));
	if (!pl)
		goto null_pl;
...
static void recycle_zone_prepare(int context)
{
	...
	int msghandle;
	if(Get_JunkZoneForceRecycleTrig(conptr->junkzone,90)){
...
/**
 *	L2PConvert_Erase  -  Erase partition
 *
 *	@handle: return value of L2PConvert_ZMOpen
*/
int L2PConvert_Erase(int handle)
{
...
PartitionInterface l2p_nand_ops = {
	...
	.PartitionInterface_Erase	= L2PConvert_Erase,
};
