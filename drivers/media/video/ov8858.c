...
#define SXGA_WIDTH		1600
#define SXGA_HEIGHT		1200
...
#if 0
static struct regval_list ov8858_init_regs[] = {
...
static struct regval_list ov8858_init_regs[] = {
    ...
    {0x3841, 0xff},
    {0x3846, 0x48},
    {0x3d85, 0x14},//OTP power up load data enable ,OTP power up load setting enable
    /* {0x3d8c, 0x73},//OTP setting start Hign */
...
static struct regval_list ov8858_win_sxga[] = {
	{OV8858_REG_END, 0x00},	/* END MARKER */
};
static struct regval_list ov8858_win_vga[] = {
	{OV8858_REG_END, 0x00},	/* END MARKER */
};

static struct regval_list ov8858_win_5m[] = {
	{OV8858_REG_END, 0x00},	/* END MARKER */
};
		...
		unsigned char *value)
{
	...
	printk("function = %s,, client->addr = %x\n",__func__ ,client->addr);
...
static int ov8858_init(struct v4l2_subdev *sd, u32 val)
{
	...
	ret = ov8858_write_array(sd, ov8858_init_regs);
	if (ret < 0)
		return ret;
	//ov8858_read_array(sd, ov8858_init_regs);
...
static int ov8858_try_fmt_internal(struct v4l2_subdev *sd,
		...
		struct ov8858_format_struct **ret_fmt,
		struct ov8858_win_size **ret_wsize)
{
	int index;
	struct ov8858_win_size *wsize;

	printk("[ov8858], %s,%d :fmt->code:%0x\n", __func__, __LINE__, fmt->code);

	for (index = 0; index < N_OV8858_FMTS; index++)
		if (ov8858_formats[index].mbus_code == fmt->code)
			break;
	if (index >= N_OV8858_FMTS) {
		/* default to first format */
		printk("[OV8858] default to first format\n");
		index = 0;
		fmt->code = ov8858_formats[0].mbus_code;
	}
	if (ret_fmt != NULL)
		*ret_fmt = ov8858_formats + index;
	...
	fmt->colorspace = ov8858_formats[index].colorspace;
...
{
	printk("%s:------->%d\n", __func__, __LINE__);
	return ov8858_try_fmt_internal(sd, fmt, NULL, NULL);
}
			  ...
			  struct v4l2_mbus_framefmt *fmt)
{
	...
	struct ov8858_format_struct *fmt_s;
	struct ov8858_win_size *wsize;
	int ret;


	printk("[ov8858], problem function:%s, line:%d\n", __func__, __LINE__);
	ret = ov8858_try_fmt_internal(sd, fmt, &fmt_s, &wsize);
	if (ret)
		return ret;
	data->vts = wsize->vts;
	data->mipi_clk = 282;
	if ((info->fmt != fmt_s) && fmt_s->regs) {
		printk("pay attention : ov8858, %s:LINE:%d\n", __func__, __LINE__);
		ret = ov8858_write_array(sd, fmt_s->regs);
		if (ret)
			return ret;
	}
		...
		struct v4l2_frmsizeenum *fsize)
{
	...
	for (i = 0; i < N_WIN_SIZES; i++) {
		struct ov8858_win_size *win = &ov8858_win_sizes[index];
		if (index == ++num_valid) {
