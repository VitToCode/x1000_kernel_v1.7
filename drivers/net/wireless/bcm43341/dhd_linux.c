...
//Peter modify start -- add
	...
	uint32 Peter_apsta = 0; /* Enable APSTA mode */
	s32 up=1;
#endif
//Peter modify end -- add

	if(DBG)printk(KERN_ERR "Peter %s: Enter\n",__FUNCTION__);
	...
	if(DBG)printk(KERN_ERR "Peter %s: point 1 -- fw_path=%s\n",__FUNCTION__,fw_path);
...
//Peter modify start -- add
	...
	wldev_ioctl(net, WLC_DOWN, &up, sizeof(s32), true);
	bcm_mkiovar("apsta", (char *)&Peter_apsta, 4, iovbuf, sizeof(iovbuf));
        if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0)) < 0) {
        	DHD_ERROR(("Peter: %s --  Set apsta failed ret= %d\n", __FUNCTION__, ret));
        }
	wldev_ioctl(net, WLC_UP, &up, sizeof(s32), true);
#endif
//Peter modify end -- add
...
exit:
	...
	if(DBG)printk(KERN_ERR "Peter %s: Exit\n",__FUNCTION__);
	return ret;
}
...
dhd_bus_start(dhd_pub_t *dhdp)
{
	...
	if  ((dhd->pub.busstate == DHD_BUS_DOWN) &&
		...
		if (!(dhd_bus_download_firmware(dhd->pub.bus, dhd->pub.osh,
			fw_path, nv_path))) {
			DHD_ERROR(("%s: dhdsdio_probe_download failed. firmware = %s \nnvram = %s\n",
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	if(DBG)printk(KERN_ERR "Peter %s: Enter\n",__FUNCTION__);
	if(DBG)printk(KERN_ERR "Peter %s: ==================start===================\n",__FUNCTION__);
	if(DBG)dump_stack();
	if(DBG)printk(KERN_ERR "Peter %s: ===================end====================\n",__FUNCTION__);
	DHD_TRACE(("Enter %s\n", __FUNCTION__));
	dhd->op_mode = 0;
#ifdef GET_CUSTOM_MAC_ENABLE
	...
	if(DBG)printk(KERN_ERR "Peter %s: Firmware=%s\n",__FUNCTION__,fw_path);
...
{
	struct file *fp;
	printk("%s :filp_open: download firmware %s\n", __FUNCTION__, filename);
