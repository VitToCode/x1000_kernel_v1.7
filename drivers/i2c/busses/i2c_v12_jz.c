...
static inline unsigned short i2c_readl(struct i2c_jz *i2c,unsigned short offset);
#ifdef I2C_DEBUG
...
static inline void i2c_writel(struct i2c_jz *i2c, unsigned short  offset, unsigned short value)
{
...
void i2c_send_rcmd(struct i2c_jz *i2c, int cmd_count)
{
	int i;
...
static inline int xfer_read(struct i2c_jz *i2c, unsigned char *buf, int len, int cnt, int idx)
{
	int ret = 0;
	long timeout;
	unsigned short tmp;
...
static inline int xfer_write(struct i2c_jz *i2c, unsigned char *buf, int len, int cnt, int idx)
{
	int ret = 0;
...
{
	struct i2c_jz *i2c = container_of(work, struct i2c_jz, clk_work.work);
	clk_disable(i2c->clk);
	i2c->enabled = 0;
}
...
static int i2c_jz_probe(struct platform_device *dev)
{
	...
	struct i2c_jz *i2c = kzalloc(sizeof(struct i2c_jz), GFP_KERNEL);
	if (!i2c) {
		ret = -ENOMEM;
		goto no_mem;
	}
	...
	ret = request_irq(i2c->irq, i2c_jz_irq, IRQF_DISABLED, dev_name(&dev->dev), i2c);
	if(ret) {
		ret = -ENODEV;
		goto irq_failed;
	}
