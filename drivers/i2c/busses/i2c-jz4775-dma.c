...
static inline int init_dma_write(struct jz_i2c *i2c,unsigned short *buf,int len)
{
	...
	unsigned char chrig_value[] = {0,0,0,0,0};
	...
	dma_cache_wback_inv((unsigned long)chrig_value, 5 * sizeof(unsigned char));
	/* use dma */
...
#ifdef USE_ONE_DESC
static inline int init_dma_read(struct jz_i2c *i2c,unsigned char *buf,int len,int cnt)
{
	struct dma_async_tx_descriptor *desc = NULL;
	dma_addr_t dma_src;
	dma_addr_t dma_dst;
	...
	dma_dst = CPHYSADDR((unsigned char*)buf);
...
static inline int init_dma_read(struct jz_i2c *i2c,unsigned char *buf,int len,int cnt)
{
	...
	/*int count = 0;*/
	struct dma_async_tx_descriptor *desc = NULL;
	unsigned short chrig_value[] = {0,0,0,0,0};
	unsigned short burst_len[] = {32,16,4,2,1};
	...
	while(length) {
		...
		chrig_value[i] = (unsigned char)burst_len[i] - 1;
		...
		dma_src = CPHYSADDR(&(chrig_value[i]));
		dma_dst = CPHYSADDR(i2c->iomem + I2C_DMARDLR);
		desc = i2c->rx_chan->device->device_add_desc(i2c->rx_chan,
				dma_src, dma_dst, chrig_value[i], DMA_TO_DEVICE, 1);
		if (desc == NULL) {
		...
		dma_src = CPHYSADDR(i2c->iomem + I2C_DC);
		dma_dst = CPHYSADDR((unsigned char*)buf + offset);
		desc = i2c->rx_chan->device->device_add_desc(i2c->rx_chan,
...
static inline int xfer_read(struct jz_i2c *i2c,unsigned char *buf,int len,int cnt,int idx)
{
	...
	chan = init_dma_read(i2c, buf, len, cnt);
	if (chan) {
