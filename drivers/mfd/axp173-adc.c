...
//		.resources = jz_battery_resources,
		...
		.enable = axp173_adc_cell_enable,
		.disable = axp173_adc_cell_disable,
	},
};
...
//	struct jz_ts_info *ts_info;
	struct axp173_adc_platform_data *adc_platform_data = pdev->dev.platform_data;
	...
	for(i = 0;i < ARRAY_SIZE(axp173_adc_cells);i++)
	{
		switch(axp173_adc_cells[i].id)
		{
			case 1:
				...
				break;
			case 2:
				axp173_adc_cells[i].platform_data = ts_info;
				break;
			default:
				break;
		}
...
//			ARRAY_SIZE(axp173_adc_cells), mem_base, adc->irq_base);
	ret = mfd_add_devices(&pdev->dev, 0, axp173_adc_cells,
...
//		goto err_clk_put;
	...
	printk("axp173 SADC driver registeres over!\n");
