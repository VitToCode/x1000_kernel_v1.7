...
#define RK_ADDR			(0x240)
#define CK_ADDR			(0x250)
#define UK_ADDR			(0x260)
#define NKU_ADDR		(0x270)
...
int jz_efuse_read(uint32_t seg_id, uint32_t data_length, uint32_t *buf)
{
	...
	switch(seg_id){
		...
		case CUT_ID:
			if(bit_num > 240) {
				dev_err(efuse->dev, "read segment %d data length %d > 240 bit", seg_id, bit_num);
				return -1;
			}
			break;
		case  PTR:
...
static int efuse_read_customer_id_proc(char *page, char **start, off_t off,
			  int count, int *eof, void *data)
{
	int len = 0;
	uint32_t buf[4];
...
static int jz_efuse_probe(struct platform_device *pdev)
{
	...
	res = create_proc_entry("efuse_cut_id", 0444, p);
	if (res) {
		res->read_proc = efuse_read_customer_id_proc;
		res->write_proc = NULL;
		res->data = NULL;
	}
