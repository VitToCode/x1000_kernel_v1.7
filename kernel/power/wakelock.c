...
#ifdef CONFIG_DUMP_PARTIAL_WAKELOCK
static struct proc_dir_entry *proc_jz_root;
static int wakelocks_debug_proc (char *page, char **start, off_t off,
...
static int __init jz_proc_init(void)
{
	...
	proc_jz_root = proc_mkdir("pmdebug", 0);

	res = create_proc_entry("wakelock", 0644, proc_jz_root);
	if (res) {
		res->read_proc = wakelocks_debug_proc;
		res->write_proc = NULL;
		res->data = NULL;
	}
